...
def init_net(net, init_type = 'normal', init_gain = 0.02, 
            gpu_ids = [], backend_pretrain = False):
    if len(gpu_ids) > 0:
...
def define_ClassifierNet(input_nc, image_size = 128, n_classes = 7, norm = 'batch', 
                        use_dropout = False, init_type = 'xavier', init_gain = 0.02, 
                        gpu_ids = [], backend = 'default', backend_pretrain = False):
    norm_layer = get_norm_layer(norm_type = norm)
    print("[Info] Norm type for Cls net is %s." % norm)
    if backend == 'inception':
        # input size 299
        net_cls = custom_inception_v3(
            pretrained = backend_pretrain, 
        ...
        # input size 224
        net_cls = torch_models.resnet50(
            pretrained = backend_pretrain, 
            num_classes = n_classes)

    elif backend == 'resnet152':
        net_cls = torch_models.resnet152(
            pretrained = backend_pretrain, 
            num_classes = n_classes)

    elif backend == 'densenet121':
        net_cls = torch_models.densenet121(
            pretrained = backend_pretrain, 
            num_classes = n_classes)
...
def define_ResFaceGenNet(input_nc, img_size, ngf = 64, norm = 'batch', use_dropout = False, 
                        n_blocks = 4, init_type = 'xavier', init_gain = 0.02, gpu_ids = []):
    norm_type = get_norm_layer(norm_type = norm)
    print("[Info] Norm type for Residual Face Genearating Net is %s." % norm)
    net_resface = ResFaceGenNet(
        input_nc, 
        img_size, 
        ngf, 
...
def define_FusionNet(input_a_nc, input_b_nc, 
...
class ResFaceGenNet(nn.Module):
    ...
    def __init__(self, input_nc, img_size = 299, ngf = 64, 
                norm_layer = nn.BatchNorm2d, use_dropout = False, 
                n_blocks = 6, padding_type = 'zero'):
        assert (n_blocks >= 0)
        ...
        for i in range(n_downsampling):
            ...
            model += [nn.ConvTranspose2d(
                        ...
                        padding = 1,
                        bias = use_bias, 
                        output_padding = trans_conv_output_padding),
...
class FusionNet(nn.Module):
    """ Created on Dec 17, 2018
        @author: Yuedong Chen """
    def __init__(self, input_a_nc = 3, input_b_nc = 1, n_blocks = 1, 
                norm_layer = nn.BatchNorm2d, use_dropout = False):
